...
export default function StevensDuckCoin({ contract, sdcContract, wallet, contractAddress }) {
  ...
  async function mintTokens() {
    ...
    try {
      ...
      await tx.wait();
      alert(`✅ Minted ${mintData.amount} Stevens Duck Coin (SDC) to ${mintData.wallet}`);
  ...
  async function burnTokens() {
    ...
    const ok = confirm(`⚠️ Are you sure you want to burn ${burnData.amount} Stevens Duck Coin (SDC) from ${burnData.wallet}?`);
    if (!ok) return;
    ...
    try {
      ...
      await tx.wait();
      alert(`🔥 Burned ${burnData.amount} Stevens Duck Coin (SDC) from ${burnData.wallet}`);
  ...
  async function transferTokens() {
    ...
    try {
      if (sdcContract) {
        ...
        await tx.wait();
        alert(`✅ Transferred ${transferData.amount} Stevens Duck Coin (SDC) to ${transferData.wallet}`);
      } else {
        ...
        {activeSubTab === "transfer" && (
          <>
            <h3 style={{ 
              ...
              💸 Transfer Stevens Duck Coin (SDC)
            </h3>
            <p style={{ marginBottom: 20, color: stevensTextGrey }}>
