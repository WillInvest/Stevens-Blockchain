...
export default function StevensBananaCoin({ contract, sbcContract, wallet, contractAddress }) {
  ...
  async function mintTokens() {
    ...
    try {
      ...
      await tx.wait();
      alert(`✅ Minted ${mintData.amount} Stevens Banana Coin (SBC) to ${mintData.wallet}`);
  ...
  async function burnTokens() {
    ...
    const ok = confirm(`⚠️ Are you sure you want to burn ${burnData.amount} Stevens Banana Coin (SBC) from ${burnData.wallet}?`);
    if (!ok) return;
    ...
    try {
      ...
      await tx.wait();
      alert(`🔥 Burned ${burnData.amount} Stevens Banana Coin (SBC) from ${burnData.wallet}`);
  ...
  async function transferTokens() {
    ...
    try {
      if (sbcContract) {
        ...
        await tx.wait();
        alert(`✅ Transferred ${transferData.amount} Stevens Banana Coin (SBC) to ${transferData.wallet}`);
      } else {
        ...
        {activeSubTab === "transfer" && (
          <>
            <h3 style={{ 
              ...
              💸 Transfer Stevens Banana Coin (SBC)
            </h3>
            <p style={{ marginBottom: 20, color: stevensTextGrey }}>
