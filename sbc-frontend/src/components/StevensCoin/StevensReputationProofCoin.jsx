...
export default function StevensReputationProofCoin({ contract, srpcContract, contractAddress }) {
  ...
  async function mintSRPC() {
    ...
    try {
      ...
      await tx.wait();
      alert(`✅ Minted ${mintData.amount} Stevens Reputation Proof Coin (SRPC) tokens to ${mintData.wallet}`);
  ...
  async function burnSRPC() {
    ...
    const ok = confirm(`⚠️ Are you sure you want to burn ${burnData.amount} Stevens Reputation Proof Coin (SRPC) tokens from ${burnData.wallet}?`);
    if (!ok) return;
    ...
    try {
      ...
      await tx.wait();
      alert(`🔥 Burned ${burnData.amount} Stevens Reputation Proof Coin (SRPC) tokens from ${burnData.wallet}`);
        ...
        {activeSubTab === "mint" && (
          <>
            <h3 style={{ 
              ...
              🪙 Mint Stevens Reputation Proof Coin (SRPC)
            </h3>
        ...
        {activeSubTab === "burn" && (
          <>
            <h3 style={{ 
              ...
              🔥 Burn Stevens Reputation Proof Coin (SRPC)
            </h3>
            <p style={{ marginBottom: 20, color: stevensTextGrey }}>
